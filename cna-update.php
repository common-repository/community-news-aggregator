...
global $wpdb;
		...
		if($community_label==false || $community_label=='') die('<status>Cannot update now. Check settings.</status>');
...
die("<response><status>ok</status><time>".$current_time_obj->format(DateTime::COOKIE)."</time></respons>");
}
...
<div class="updated" id="cna-status-container" hidden="true"><img id="cna-loading" src="<?php echo $plugin_url.'loading.gif' ?>"/><span id="cna-status">Updating</span></div>
	<p>Last Update: <b><span id="cna-update-time"><?php echo $last_in->format(DateTime::COOKIE);?></span></b></p>
